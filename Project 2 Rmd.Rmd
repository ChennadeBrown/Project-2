...
# Remove extra columns added to onLineNews.
...
### Random Forest Model
Random Forest models are an extension of the tree based method bagging.  The random forest algorithm creates multiple trees from bootstrapped samples, uses a random subset of predictors for each bootstrap sample/tree fit, and predicts by averaging the results from various trees.  Random forests can be used for classification and regression problems.
...
# Use the apply function to return the winner (model with the lowest RMSE).
