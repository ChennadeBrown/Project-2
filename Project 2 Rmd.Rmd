...
fit <- train(model, data = onlineNewsTrain,
              method = "lm",
              preProcess = c("center", "scale"),
              trControl = trainControl(method = "cv", number = 10))
```
...
pred <- predict(fit, newdata = onlineNewsTest)
p <- postResample(pred, obs = onlineNewsTest$shares)

# Test error computed for boosted tree model
pred <- predict(boostedTree, newdata = onlineNewsTest)
p2 <- postResample(pred, obs = onlineNewsTest$shares)
